...
func CurrentWeather(city string, api_key string) {
	...
	if err != nil {
		log.Println(err.Error(), "Error in response")
		return
	}
	...
	if err != nil {
		log.Println(err.Error(), "Error in reading data")
		return
	}
	...
	if err != nil {
		log.Println(err.Error(), "Error in struct insert")
		return
	}
	if len(weather.Weather) > 0 {
		log.Printf("Today's temperature in %s is %.2f°C, weather condition is %s\n", city, weather.Main.Temp, weather.Weather[0].Main)
	} else {
		log.Printf("Today's temperature in %s is %.2f°C, but no weather condition data available\n", city, weather.Main.Temp)
	}
}
